...
// Validar que existe el parámetro id
...
// Consulta con columnas existentes en la tabla requerimientos
$query_seguimiento = "SELECT sa.*, a.nombre as actividad_nombre, a.orden, a.tiempo_dias, 
                             r.id as requerimiento_id, r.codigo, r.area_id, r.observaciones as requerimiento_observaciones,
...
// Si no existe el seguimiento, redirigir a requerimientos
if (!$seguimiento) {
    redirectTo('modules/requerimientos/requerimientos.php');
    exit();
}
...
if ($_POST) {
    try {
        ...
        // Obtener información del usuario actual para el correo
        $query_usuario_actual = "SELECT nombre, email FROM usuarios WHERE id = ?";
        $stmt_usuario_actual = $db->prepare($query_usuario_actual);
        $stmt_usuario_actual->execute([$usuario_id]);
        $usuario_actual = $stmt_usuario_actual->fetch(PDO::FETCH_ASSOC);
        
        // ENVÍO DE NOTIFICACIÓN POR CORREO USANDO LA FUNCIÓN EXISTENTE
        $correo_enviado = false;
        
        // Preparar datos para la notificación
        $datos_notificacion = [
            'actividad_nombre' => $seguimiento['actividad_nombre'],
            'requerimiento_codigo' => $seguimiento['codigo'],
            'proceso_nombre' => $seguimiento['proceso_nombre'],
            'area_nombre' => $seguimiento['area_nombre'],
            'estado_anterior' => ucfirst($seguimiento['estado']),
            'estado_nuevo' => ucfirst($estado),
            'observaciones' => $observaciones,
            'usuario_nombre' => $usuario_actual['nombre'],
            'fecha_actualizacion' => date('d/m/Y H:i'),
            'descripcion_requerimiento' => !empty($seguimiento['requerimiento_observaciones']) 
                ? substr(strip_tags($seguimiento['requerimiento_observaciones']), 0, 100) . '...' 
                : 'Sin descripción detallada'
        ];
        
        // Obtener destinatarios
        $query_destinatarios = "SELECT u.email 
                               FROM usuarios u 
                               WHERE u.rol IN ('admin', 'supervisor') 
                               AND (u.area_id = ? OR u.rol = 'admin')
                               AND u.email IS NOT NULL";
        $stmt_dest = $db->prepare($query_destinatarios);
        $stmt_dest->execute([$seguimiento['area_id']]);
        $destinatarios = $stmt_dest->fetchAll(PDO::FETCH_COLUMN);
        
        error_log("=== DEBUG NOTIFICACIÓN ===");
        error_log("Destinatarios encontrados: " . implode(', ', $destinatarios));
        error_log("Tipo notificación: actualizacion_actividad");
        error_log("Datos: " . print_r($datos_notificacion, true));
        ...
        if (!empty($destinatarios)) {
            ...
            error_log("Configuración SMTP: " . ($config ? 'EXISTE' : 'NO EXISTE'));
            if ($config) {
                error_log("Email activo: " . ($config['email_activo'] ? 'SÍ' : 'NO'));
                error_log("SMTP Host: " . ($config['smtp_host'] ?? 'No configurado'));
            }
            
            // Enviar notificación usando la función existente
            $correo_enviado = enviarNotificacionEmail(
                'actualizacion_actividad', 
                $datos_notificacion, 
                $destinatarios
            );
            
            error_log("Resultado envío: " . ($correo_enviado ? 'EXITOSO' : 'FALLIDO'));
        } else {
            error_log("No se encontraron destinatarios para notificación");
        }
        ...
        if ($estado == 'completado') {
            ...
            if ($siguiente_actividad) {
                ...
                // Enviar notificación para nueva actividad creada
                $datos_nueva_actividad = [
                    'actividad_nombre' => 'Próxima actividad del proceso',
                    'requerimiento_codigo' => $seguimiento['codigo'],
                    'proceso_nombre' => $seguimiento['proceso_nombre'],
                    'area_nombre' => $seguimiento['area_nombre'],
                    'usuario_nombre' => $usuario_actual['nombre'],
                ...
                enviarNotificacionEmail('nueva_actividad_creada', $datos_nueva_actividad, $destinatarios);
            }
        ...
        if ($estado == 'completado') {
            ...
            if ($resultado_ultima['total'] == 0) {
                ...
                // Enviar notificación de requerimiento completado
                $datos_completado = [
                    'requerimiento_codigo' => $seguimiento['codigo'],
                    'proceso_nombre' => $seguimiento['proceso_nombre'],
                    'area_nombre' => $seguimiento['area_nombre'],
                ...
                enviarNotificacionEmail('requerimiento_completado', $datos_completado, $destinatarios);
            }
        }
        ...
        redirectTo("modules/requerimientos/requerimientos.php?mensaje=" . urlencode($mensaje));
        exit();
...
<head>
    ...
    <link href="/opticap/assets/fontawesome/css/all.min.css" rel="stylesheet">
</head>
<body>
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    ...
                    <a href="modules/requerimientos/requerimientos.php" class="btn btn-outline-secondary">
                        <i class="fas fa-arrow-left me-1"></i> Volver a Requerimientos
                    </a>
                </div>
                            ...
                            <div class="card-body">
                                <form method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                                        ...
                                        <a href="modules/requerimientos/requerimientos.php" class="btn btn-outline-secondary">Cancelar</a>
                                    </div>
                                </form>
                            </div>
                        </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="card mt-3">
                            <div class="card-header bg-warning text-dark">
                                <h6 class="card-title mb-0">Notificaciones</h6>
                            </div>
                            <div class="card-body">
                                <p class="small">
                                    <i class="fas fa-info-circle me-1"></i>
                                    Al actualizar esta actividad, se enviará una notificación por correo a los supervisores del área y administradores del sistema.
                                </p>
                                <p class="small text-muted">
                                    <i class="fas fa-envelope me-1"></i>
                                    Sistema de notificaciones usando PHPMailer configurado.
                                </p>
                                <p class="small text-info">
                                    <i class="fas fa-bug me-1"></i>
                                    Revisa los logs para ver el estado del envío.
                                </p>
                            </div>
                        </div>
                    </div>
